...
router.post('/daily', async (req, res) => {
    try {
        ...
        const maxSequence = await RashifalDailyDate.findOne().sort({ sequence: -1 }).select('sequence');
        const sequence = maxSequence && maxSequence.sequence ? maxSequence.sequence + 1 : 1;
        
        const created = await RashifalDailyDate.create({ dateLabel, dateISO, notes, sequence });
...
router.delete('/daily', async (req, res) => {
    try {
        ...
        const dates = await RashifalDailyDate.find({ _id: { $in: dateIds } });
        ...
        await RashifalDailyContent.deleteMany({ dateRef: { $in: dateObjectIds } });
        
        // Delete the dates
        const result = await RashifalDailyDate.deleteMany({ _id: { $in: dateObjectIds } });
...
// Get all years (monthly rashifal years)
router.get('/months', async (req, res) => {
    try {
        const years = await RashifalMonthlyYear.find().sort({ year: -1 });
...
router.get('/months/:yearId', async (req, res) => {
    try {
        ...
        if (/^\d+$/.test(yearId)) {
            ...
            yearDoc = await RashifalMonthlyYear.findOne({ id: parseInt(yearId) });
        } else {
            // Otherwise find by _id
            yearDoc = await RashifalMonthlyYear.findById(yearId);
        }
        ...
        for (let i = 0; i < monthNames.length; i++) {
            ...
            const contents = await RashifalMonthly.find({ 
                yearRef: yearDoc._id, 
                month: monthName 
            }).sort({ sequence: 1 });
...
router.get('/months/:yearId/:month', async (req, res) => {
    try {
        ...
        console.error('Error fetching monthly rashifal:', error);
        res.status(500).json({
            success: false,
...
router.get('/months/:yearId/:month/:contentId', async (req, res) => {
    try {
        ...
        if (/^\d+$/.test(contentId)) {
            ...
            content = await RashifalMonthly.findById(contentId);
...
router.get('/yearly', async (req, res) => {
    try {
        const years = await RashifalYearlyYear.find()
...
router.get('/yearly/:yearId', async (req, res) => {
    try {
        ...
        if (yearId.match(/^[0-9a-fA-F]{24}$/)) {
            // MongoDB ObjectId
            year = await RashifalYearlyYear.findById(yearId);
        } else {
            // Numeric id
            year = await RashifalYearlyYear.findOne({ id: parseInt(yearId) });
        }
        ...
        const yearlyContent = await RashifalYearly.find({ yearRef: year._id })
