...
HOW TO PRESENT DATABASE RESULTS:
  • KOSH: Show the title, then show the EXACT meaning from database. Then COPY the "location" field from the snippets and show it to the user. The location field already has the full path like "📍 Kosh → Category → SubCategory". You MUST include this location line. It is MANDATORY.
  • KARMKAND: Same — show title, EXACT meaning, then COPY the "location" field from snippets. MANDATORY.
...
function buildAppSearchContext(searchResult, appDataLikely) {
  ...
  if (found && totalMatches > 0) {
    lines.push('CRITICAL: Your answer MUST use the "snippets" content above. Do NOT use your own knowledge. If a snippet has a "location" field, you MUST copy and show that location line to the user.');
  }
